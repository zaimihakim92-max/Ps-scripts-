...
<#
.SYNOPSIS
    Ajout / retrait en masse d'utilisateurs dans un groupe Microsoft 365 (Entra ID) via le module Az.

.DESCRIPTION
    Interface graphique WinForms. A lancer depuis une console PowerShell ou la connexion Azure est
    deja etablie (Connect-AzAccount effectue, module Az.Resources disponible). Le script N'UTILISE PAS
    Microsoft Graph directement : il s'appuie sur les cmdlets Az (Get-AzADGroup, Get-AzADUser,
    ...
    Concu pour ne laisser AUCUNE place a une action indesirable :
      - le contexte Azure est detecte, jamais reconnecte a l'aveugle ;
      - le groupe cible doit etre VALIDE avant toute action ;
      - toute modification de la cible (groupe ou liste d'utilisateurs) re-arme les verrous ;
      - une RECHERCHE prealable est obligatoire avant toute execution ;
      - les entrees sont sanitisees : aucun caractere generique ( * ? % [ ] ) n'est accepte ;
      - resolution stricte : un nom ambigu (plusieurs objets) est IGNORE, jamais devine ;
      - confirmation detaillee avant l'ajout ; confirmation par SAISIE DE L'ID avant le retrait ;
      - les groupes DYNAMIQUES sont refuses (appartenance geree par regle) ;
      - journalisation horodatee (GUI + console + fichier) et export CSV.

.NOTES
    Lancer de preference via :  powershell.exe -STA -File .\M365_GroupBulk_GUI.ps1
    (ou depuis la meme console ou Connect-AzAccount a ete lance, pour conserver le contexte).
#>
...
if ([System.Threading.Thread]::CurrentThread.GetApartmentState() -ne 'STA') {
    ...
    Write-Warning "Relancez avec :  powershell.exe -STA -File .\M365_GroupBulk_GUI.ps1"
}

# ============================================================================
#  1. Etat global
# ============================================================================
$script:AzAccount      = $null
$script:AzTenant       = $null
$script:ValidatedGroup = $null      # objet : Id / Name / Type / IsDynamic
$script:SimData        = $null
...
function Initialize-LogFile {
    ...
    Write-Log "Operateur : $env:USERNAME  /  Compte Azure : $($script:AzAccount)" 'INFO'
}
...
function Initialize-AzContext {
    ...
    if (-not $ctx -or -not $ctx.Account) { return $false }
    $script:AzAccount = $ctx.Account.Id
    $script:AzTenant  = $ctx.Tenant.Id
    return $true
}
...
function Test-UpnSafe {
    # Format UPN / e-mail. Rejette tout caractere generique.
    param([string] $Name)
...
function Validate-Group {
    ...
    if ([string]::IsNullOrWhiteSpace($target)) {
        [System.Windows.Forms.MessageBox]::Show("Saisissez un nom ou un ObjectId de groupe.","Validation",'OK','Warning') | Out-Null
        return
    }
    ...
    try {
        if ($isGuid) {
            $grp = Get-AzADGroup -ObjectId $target -ErrorAction SilentlyContinue
        } else {
            $matches = @(Get-AzADGroup -DisplayName $target -ErrorAction SilentlyContinue | Where-Object { $_.DisplayName -eq $target })
            if ($matches.Count -gt 1) {
                $ids = ($matches | ForEach-Object { $_.Id }) -join ', '
                Write-Log "Nom ambigu ($($matches.Count) groupes) : $ids. Utilisez l'ObjectId." 'ERR'
                $script:lblGroupInfo.Text      = "Nom ambigu ($($matches.Count) groupes). Utilisez l'ObjectId : $ids"
    ...
    # Enrichissement du type (groupTypes n'est pas retourne par defaut).
    $gtypes = @()
    try {
        $full = Get-AzADGroup -ObjectId $gid -Select 'groupTypes' -AppendSelected -ErrorAction SilentlyContinue
        if ($full) {
            if     ($full.PSObject.Properties['GroupType'])  { $gtypes = @($full.GroupType) }
            elseif ($full.PSObject.Properties['GroupTypes']) { $gtypes = @($full.GroupTypes) }
        }
    } catch { }

    $isDynamic = ($gtypes -contains 'DynamicMembership')
    $isUnified = ($gtypes -contains 'Unified')
    $secEnabled  = [bool]$grp.SecurityEnabled
    $mailEnabled = [bool]$grp.MailEnabled

    $typeLabel = if ($isUnified) { 'Microsoft 365' }
                 elseif ($secEnabled -and -not $mailEnabled) { 'Securite' }
                 elseif ($mailEnabled -and -not $secEnabled) { 'Distribution' }
                 else { 'Autre' }

    # Verrou : un groupe dynamique ne peut pas etre modifie manuellement.
    if ($isDynamic) {
        Write-Log "Groupe DYNAMIQUE ($gid) : ajout/retrait manuel impossible (appartenance geree par regle)." 'ERR'
        $script:lblGroupInfo.Text      = "Groupe DYNAMIQUE ($typeLabel) : appartenance geree par regle, ajout/retrait manuel interdit."
        $script:lblGroupInfo.ForeColor = [System.Drawing.Color]::Firebrick
        return
    }
    ...
    $info = "VALIDE  |  Nom : $($grp.DisplayName)  |  ObjectId : $gid  |  Type : $typeLabel  |  Appartenance : Assignee"
    $script:lblGroupInfo.Text      = $info
    $script:lblGroupInfo.ForeColor = [System.Drawing.Color]::DarkGreen
    Write-Log $info 'OK'
    if ($typeLabel -eq 'Distribution') {
        Write-Log "Attention : liste de distribution. Les cmdlets Az peuvent ne pas gerer ce type (gestion Exchange)." 'WARN'
    }
...
function Invoke-Simulation {
    ...
    try {
        # Ensemble frais des ObjectId deja membres.
        $memberSet = New-Object 'System.Collections.Generic.HashSet[string]'
        foreach ($m in @(Get-AzADGroupMember -GroupObjectId $gid -ErrorAction SilentlyContinue)) {
            if ($m.Id) { [void]$memberSet.Add([string]$m.Id) }
        }
        Write-Log "Membres actuels du groupe : $($memberSet.Count)." 'RECH'
        ...
        foreach ($name in $names) {
            ...
            if (-not (Test-UpnSafe $name)) {
                $data.Add([pscustomobject]@{ Name=$name; Category='INVALIDE'; ObjectId=$null; DisplayName=''; Detail='Format UPN/e-mail invalide' })
                Write-Log "INVALIDE : '$name'" 'WARN'
                Add-GridRow $name 'INVALIDE' '' '' 'Format invalide'
                [System.Windows.Forms.Application]::DoEvents(); continue
            }

            $users = @(Get-AzADUser -UserPrincipalName $name -ErrorAction SilentlyContinue)
            if ($users.Count -eq 0) {
                # Comptes invites (#EXT#) : l'email externe n'est ni l'UPN, ni fiable via -Mail
                # (bug connu : -Mail filtre en realite sur l'UPN). On resout par mail puis otherMails
                # (adresse d'invitation), en requete avancee (ConsistencyLevel eventual).
                $esc = $name.Replace("'","''")
                try {
                    $users = @(Get-AzADUser -Filter "mail eq '$esc'" -ConsistencyLevel eventual -ErrorAction Stop)
                    if ($users.Count -eq 0) {
                        $users = @(Get-AzADUser -Filter "otherMails/any(c:c eq '$esc')" -ConsistencyLevel eventual -Count -ErrorAction Stop)
                    }
                } catch {
                    Write-Log "Resolution invite impossible pour '$name' : $($_.Exception.Message)" 'WARN'
                    $users = @()
                }
            }

            if ($users.Count -eq 0) {
                $data.Add([pscustomobject]@{ Name=$name; Category='NON_TROUVE'; ObjectId=$null; DisplayName=''; Detail='Aucun compte' })
                Write-Log "NON TROUVE : '$name'" 'WARN'
                Add-GridRow $name 'NON_TROUVE' '' '' 'Aucun compte'
            }
            elseif ($users.Count -gt 1) {
                $ids = ($users | ForEach-Object { $_.Id }) -join ','
                $data.Add([pscustomobject]@{ Name=$name; Category='AMBIGU'; ObjectId=$null; DisplayName=''; Detail="ObjectId multiples : $ids" })
                Write-Log "AMBIGU : '$name' -> $($users.Count) objets ($ids). IGNORE." 'WARN'
                Add-GridRow $name 'AMBIGU' '' '' "Multiples : $ids"
            }
            else {
                $uid = [string]$users[0].Id
                $dn  = [string]$users[0].DisplayName
                if ($memberSet.Contains($uid)) {
                    $data.Add([pscustomobject]@{ Name=$name; Category='MEMBRE'; ObjectId=$uid; DisplayName=$dn; Detail='Deja membre' })
                    Add-GridRow $name 'MEMBRE' $uid $dn 'Deja membre'
                } else {
                    $data.Add([pscustomobject]@{ Name=$name; Category='HORS_GROUPE'; ObjectId=$uid; DisplayName=$dn; Detail='Non membre' })
...
function Show-GroupMembers {
    ...
    $loadData = {
        ...
        $members = @()
        try {
            $members = @(Get-AzADGroupMember -GroupObjectId $gid -ErrorAction SilentlyContinue)
        }
        ...
        foreach ($m in $members) {
            # UPN et type d'objet en best-effort (selon la version du module).
            $upn = ''
            if ($m.PSObject.Properties['UserPrincipalName'] -and $m.UserPrincipalName) { $upn = [string]$m.UserPrincipalName }
            elseif ($m.PSObject.Properties['AdditionalProperties'] -and $m.AdditionalProperties -and $m.AdditionalProperties['userPrincipalName']) { $upn = [string]$m.AdditionalProperties['userPrincipalName'] }

            $otype = ''
            if ($m.PSObject.Properties['OdataType'] -and $m.OdataType) { $otype = [string]$m.OdataType }
            elseif ($m.PSObject.Properties['AdditionalProperties'] -and $m.AdditionalProperties -and $m.AdditionalProperties['@odata.type']) { $otype = [string]$m.AdditionalProperties['@odata.type'] }
            $otype = $otype -replace '#microsoft.graph.',''

            $rows.Add([pscustomobject]@{
                Member      = $m
                DisplayName = [string]$m.DisplayName
                UPN         = $upn
                ObjectId    = [string]$m.Id
                Type        = $otype
            })
        }
    ...
    $lblFilter.Text = "Filtre (nom/UPN) :"; $lblFilter.SetBounds(12,36,110,22)
    $txtFilter = New-Object System.Windows.Forms.TextBox
    $txtFilter.SetBounds(125,33,683,24); $txtFilter.Anchor = 'Top,Left,Right'
    ...
    $render = {
        ...
        foreach ($r in $rows) {
            if ($flt) {
                $f2 = [regex]::Escape($flt)
                if (($r.DisplayName -notmatch $f2) -and ($r.UPN -notmatch $f2)) { continue }
            }
            $vals = New-Object System.Collections.Generic.List[object]
            $vals.Add($r.DisplayName); $vals.Add($r.UPN); $vals.Add($r.ObjectId); $vals.Add($r.Type)
    ...
    $btnAddCol.Add_Click({
        ...
        $already = @('DisplayName','Id','UserPrincipalName') + ($extraCols | ForEach-Object { $_.Prop })
        ...
        if ($sfd.ShowDialog() -eq 'OK') {
            try {
                $out = foreach ($r in $rows) {
                    $o = [ordered]@{ 'Nom affiche'=$r.DisplayName; UPN=$r.UPN; ObjectId=$r.ObjectId; Type=$r.Type }
                    foreach ($ec in $extraCols) {
                        $v = $null
                        try { $v = $r.Member.$($ec.Prop) } catch { }
                        if ($v -is [array]) { $v = ($v -join '; ') }
    ...
    $btnRemoveSel.Add_Click({
        ...
        foreach ($oid in $toRemove) {
            try {
                Remove-AzADGroupMember -GroupObjectId $gid -MemberObjectId $oid -ErrorAction Stop -Confirm:$false | Out-Null
...
function Invoke-Operation {
    ...
    foreach ($t in $targets) {
        ...
        try {
            if ($Action -eq 'Add') {
                Add-AzADGroupMember -TargetGroupObjectId $gid -MemberObjectId $t.ObjectId -ErrorAction Stop -Confirm:$false | Out-Null
                Write-Log "OK  AJOUT  $($t.Name) ($($t.ObjectId))" 'OK'
            } else {
                Remove-AzADGroupMember -GroupObjectId $gid -MemberObjectId $t.ObjectId -ErrorAction Stop -Confirm:$false | Out-Null
                Write-Log "OK  RETRAIT  $($t.Name) ($($t.ObjectId))" 'OK'
            }
            $ok++
            ...
            $fail++
            Write-Log "ECHEC  $($t.Name) ($($t.ObjectId)) : $($_.Exception.Message)" 'ERR'
...
$script:MainForm.Text = "Microsoft 365 - Ajout / retrait d'utilisateurs dans un groupe (via Az)"
...
$lbl1.Text = "Nom ou ObjectId :"; $lbl1.SetBounds(15,28,105,20)
$script:txtGroup = New-Object System.Windows.Forms.TextBox
$script:txtGroup.SetBounds(125,25,400,24); $script:txtGroup.Anchor = 'Top,Left,Right'
...
$script:btnExport.Add_Click({
    ...
    if ($sfd.ShowDialog() -eq 'OK') {
        try {
            $script:SimData | Select-Object Name,Category,ObjectId,DisplayName,Detail |
...
#  12. Demarrage
# ============================================================================
if (-not (Initialize-AzContext)) {
    [System.Windows.Forms.MessageBox]::Show(
        "Aucun contexte Azure detecte.`n`n" +
        "Lancez ce script depuis une console ou Connect-AzAccount a ete effectue " +
        "et le module Az.Resources est disponible.",
        "Contexte Azure manquant",'OK','Error') | Out-Null
    $lblCtx.Text = "Contexte Azure : NON DETECTE - actions bloquees."
    $lblCtx.ForeColor = [System.Drawing.Color]::Firebrick
    $script:btnValidate.Enabled = $false
} else {
    $lblCtx.Text = "Contexte Azure detecte - Compte : $($script:AzAccount)   |   Tenant : $($script:AzTenant)"
